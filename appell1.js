        ...
        function vueAppell(e) {
            // La fonction vueAppell est déclenchée lorsqu'un événement de saisie est déclenché
            ...
            document.getElementById("txtappell_vue").value = valeurs[1];
            // document.getElementById("txtappell_vue").value = valeurs[2];
            // document.getElementById("txtemail_vue").value = valeurs[3];
        }


        function supprAppell() {
            // La fonction supprAppell est définie ici.
            ...
            tableBody.addEventListener("click", function (event) { // ici je rend la table entière (générée au préalable of course) sensible au click
                ...
                if (event.target.classList.contains("suppr_appell")) { // je track les click sur les boutons suppr en utilisant l'event
                    // Si la classe "suppr_appell" est présente dans la liste des classes de l'élément cible de l'événement...  
                    ...
                    alert ("Souhaitez vous le supprimer?")
                    ...
                    fetch(urlApiAppell + "/" + code, requestOptionsDelete)
        ...
        function enregAjoutAppell() {
            alert("Ajout enregistrer");
            // La fonction enregAjoutAppell est définie ici.
            let nouvelleLigne = {
                NOMAPPELLATION: document.getElementById("txtappell_ajout").value,
            ...
            fetch(urlApiAppell, requestOptionsAdd)
                .then((response) => response.json())
                .then(function (data) {
                    location.reload();
                })
        ...
        function modifAppell(event) {
            ...
            let appell = document.getElementById("txtappell_modif");
            // L'élément avec l'ID "txtappell_modif" est stocké dans la variable "cepage".
            let code = row.childNodes[0].textContent;
            // Le contenu textuel du premier enfant de la ligne est stocké dans la variable "code".
            appell.value = row.childNodes[1].textContent;
            ...
            function enregistrerModif() {
                ...
                fetch(urlApiAppell + "/" + code, requestOptionsModif)
                    .then((response) => response.json())
                    .then(function (data) {
        ...
        const boutonModif = document.getElementsByClassName("mod_appell");
        // Les éléments avec la classe "mod_appell" sont stockés dans la variable "boutonModif".
        // Ajouter l'événement click à chaque élément de la collection
        for (let i = 0; i < boutonModif.length; i++) {
            boutonModif[i].addEventListener("click", modifAppell, false);
        }
        ...
        function afficherTableau() {
            ...
            tab.header = ["Code Appellation", "Appellation"];
            // La propriété "header" de l'objet "tab" est définie avec un tableau contenant les en-têtes de colonnes.
            tab.class_modif = "mod_appell";
            tab.class_suppr = "suppr_appell";
            tab.class_vue = "vue_appell";
            ...
            // Les méthodes
            tab.fonction_modif = modifAppell;
            tab.fonction_suppr = supprAppell;
            tab.fonction_vue = vueAppell;
            ...
            fetch(urlApiAppell, requestOptions)
                .then((response) => response.json())
                .then(function (data) {
                    tab.data = data.APPELLATION.records;
                    tab.generer();
                })
                .catch(function (error) {
                    alert("Ajax error: " + error);
            ...
            document
                ...
                .addEventListener("click", enregAjoutAppell, false);
        }
        // Un écouteur d'événements click est ajouté à l'élément avec l'ID "btnAjout_enreg".
        // Lorsque le bouton est cliqué, la fonction enregAjoutAppell est appelée.
        ...
        window.addEventListener("load", (event) => {
            ...
            // Les éléments avec la classe "mod_appell" sont stockés dans la variable "boutonModif".
            for (let i = 0; i < boutonModif.length; i++) {
                boutonModif[i].addEventListener("click", modifAppell);
                // Un écouteur d'événements click est ajouté à chaque élément dans la collection "boutonModif".
                // Lorsque l'un des boutons est cliqué, la fonction modifAppell est appelée.
            }
        });
