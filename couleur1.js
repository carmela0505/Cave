        ...
        function vueCouleur(e) {
            // La fonction vueCouleur est déclenchée lorsqu'un événement de saisie est déclenché
            ...
            document.getElementById("txtcouleur_vue").value = valeurs[1];
            // document.getElementById("txtcouleur_vue").value = valeurs[2];
            // document.getElementById("txtemail_vue").value = valeurs[3];
        }


        function supprCouleur() {
            // La fonction Couleur est définie ici.
            ...
            tableBody.addEventListener("click", function (event) { // ici je rend la table entière (générée au préalable of course) sensible au click
                ...
                if (event.target.classList.contains("suppr_couleur")) { // je track les click sur les boutons suppr en utilisant l'event
                    if (confirm('Voulez-vous supprimer ce vin ?')) {
                    // Si la classe "suppr_couleur" est présente dans la liste des classes de l'élément cible de l'événement...  
                    ...
                    fetch(urlApiCouleur + "/" + code, requestOptionsDelete)
        ...
        function enregAjoutCouleur() {
            alert("Ajout enregistrer");
            // La fonction enregAjoutCouleur est définie ici.
            let nouvelleLigne = {
                NOMCOULEUR: document.getElementById("txtcouleur_ajout").value,
                // Un nouvel objet "nouvelleLigne" est créé avec une propriété "NOMAPPELLATION" ayant la valeur de l'élément avec l'ID "txtcouleur_ajout".
        ...
        function modifCouleur(event) {
            ...
            let couleur = document.getElementById("txtcouleur_modif");
            // L'élément avec l'ID "txtcouleur_modif" est stocké dans la variable "cepage".
            let code = row.childNodes[0].textContent;
            // Le contenu textuel du premier enfant de la ligne est stocké dans la variable "code".
            couleur.value = row.childNodes[1].textContent;
            ...
            function enregistrerModif() {
                ...
                fetch(urlApiCouleur + "/" + code, requestOptionsModif)
                    .then((response) => response.json())
                    .then(function (data) {
        ...
        const boutonModif = document.getElementsByClassName("mod_couleur");
        // Les éléments avec la classe "mod_couleur" sont stockés dans la variable "boutonModif".
        // Ajouter l'événement click à chaque élément de la collection
        for (let i = 0; i < boutonModif.length; i++) {
            boutonModif[i].addEventListener("click", modifCouleur, false);
        }
        ...
        function afficherTableau() {
            ...
            tab.header = ["Code Couleur", "Couleur"];
            // La propriété "header" de l'objet "tab" est définie avec un tableau contenant les en-têtes de colonnes.
            tab.class_modif = "mod_couleur";
            tab.class_suppr = "suppr_couleur";
            tab.class_vue = "vue_couleur";
            ...
            // Les méthodes
            tab.fonction_modif = modifCouleur;
            tab.fonction_suppr = supprCouleur;
            tab.fonction_vue = vueCouleur;
            ...
            document
                ...
                .addEventListener("click", enregAjoutCouleur, false);
        }
        // Un écouteur d'événements click est ajouté à l'élément avec l'ID "btnAjout_enreg".
        // Lorsque le bouton est cliqué, la fonction enregAjoutCouleur est appelée.
        ...
        window.addEventListener("load", (event) => {
            ...
            // Les éléments avec la classe "mod_couleur" sont stockés dans la variable "boutonModif".
            for (let i = 0; i < boutonModif.length; i++) {
                boutonModif[i].addEventListener("click", modifCouleur);
                // Un écouteur d'événements click est ajouté à chaque élément dans la collection "boutonModif".
                // Lorsque l'un des boutons est cliqué, la foncCouleur est appelée.
            }
        });
